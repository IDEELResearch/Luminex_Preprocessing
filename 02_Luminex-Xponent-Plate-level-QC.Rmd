...
date: "2024-06-04"
params:
  input_file: "data/raw/luminex/RTSS_Ahero_IgG3_3.21.24_Lowbeadtest.csv" 
...
# Extract the beadqc_df data frame
...
# Generate the heatmap if beadqc_df is not NULL
if (!is.null(beadqc_df)) {
  # Convert the dataframe to long format
  beadqc_long <- beadqc_df %>% pivot_longer(cols = -c(Location, Sample), names_to = "Numeric_Column", values_to = "Value")
  ...
  plot <- ggplot(beadqc_long, aes(x = Numeric_Column, y = factor(Location, levels = rev(unique(beadqc_long$Location))), fill = Value)) +
    ...
    scale_fill_gradient(low = "red", high = "blue", limits = c(0, max(beadqc_long$Value, na.rm = TRUE)), breaks = seq(0, max(beadqc_long$Value, na.rm = TRUE), by = 50)) +
...
# No input required
       ...
       # Replace bead counts below the minimum threshold as "Failed QC (bead)"
          for (col in names(beadqc_df)[sapply(beadqc_df, is.numeric)]) {
            beadqc_df[[col]][beadqc_df[[col]] < params$min_beadcount] <- "Failed QC (bead)"
          }
          ...
          # Export the dataframe of low wells as a CSV file
...
#No input required
      # Median MFI ----
            # medianMFI_df: File Extraction, initial triming, BSA subtraction for Median MFI data ---- 
            ...
            # Remove low bead values as NA, using beadqc_low_df
                  for (i in 1:nrow(beadqc_low_df)) {
                    ...
                    # Find the row index in medianMFI_df where Location and Sample match
                    row_index <- which(medianMFI_df$Location == location & medianMFI_df$Sample == sample)
                    
                    # Remove value in specified column for the found row index
                    medianMFI_df[row_index, column] <- NA
                  }
                  
            # Creation of medianMFI_df: Filter out columns between "Location" and "Total.Events"
            ...
            medianMFI_df <- medianMFI_df[, columns_to_keep]
            
        # BSA Subtraction
           # Assuming BSA column index is known
            bsa_index <- grep("BSA", colnames(medianMFI_df))
            
                  # Store the BSA column separately
                  bsa_values <- medianMFI_df[, bsa_index]
                  
                  # Identify numerical columns excluding BSA
                  nonBSA_numerical_columns <- sapply(medianMFI_df[, -bsa_index], is.numeric)
            
            # Subtract BSA from numerical columns
            medianMFI_df[, nonBSA_numerical_columns] <- medianMFI_df[, nonBSA_numerical_columns] - bsa_values
              
                  # Reinsert the BSA column
                  medianMFI_df[, bsa_index] <- bsa_values
```
...
# No input required
        # Analytes: Get the names of numerical columns
          analytes <- names(medianMFI_df)[sapply(medianMFI_df, is.numeric)]
        
        # Filter rows with "Background" and without "Background" in Sample column
            background_rows <- medianMFI_df %>%
              filter(grepl("^Background", Sample, ignore.case = TRUE))
            non_background_rows <- medianMFI_df %>%
              filter(!grepl("^Background", Sample, ignore.case = TRUE))
