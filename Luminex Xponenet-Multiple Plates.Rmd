...
# Write the standard QC results to a CSV file
output_standard_qc <- file.path("data/qc", "all_plates_standardsqc.csv")
...
# Write the compiled bead QC data to a CSV file
output_beadqc <- file.path("data/qc", "all_plates_beadqc.csv")
...
# List all clean CSV files in the directory
clean_files <- list.files("data/clean/luminex/", pattern = "_clean\\.csv$", full.names = TRUE)

# Read and rename each clean file
for (clean_file in clean_files) {
  clean_data <- read.csv(clean_file)
  
  # Rename the analyte columns
  renamed_data <- rename_analyte_columns(clean_data, basename(clean_file))
  
  # Write the renamed data back to the same file
  write.csv(renamed_data, file = clean_file, row.names = FALSE)
  cat("Renamed columns for file:", clean_file, "\n")
}

# Step 1: Read Clean Files
clean_files <- list.files("data/clean/luminex/", pattern = "_clean\\.csv$", full.names = TRUE)
clean_data <- lapply(clean_files, read.csv)

# Step 2: Remove Location Column
clean_data <- lapply(clean_data, function(df) df[, !grepl("^Location$", colnames(df))])

# Step 3: Separate Standard Samples
standard_samples <- lapply(clean_data, function(df) df[grep("standard", df$Sample, ignore.case = TRUE), ])
non_standard_samples <- lapply(clean_data, function(df) df[!grepl("standard", df$Sample, ignore.case = TRUE), ])

# Step 4: Aggregate Non-Standard Samples
aggregate_non_standard <- lapply(non_standard_samples, function(df) {
  aggregate(df[, -1], by = list(Sample = df$Sample), mean, na.rm = TRUE)
})

# Step 5: Aggregate Standard Samples
aggregate_standard <- lapply(standard_samples, function(df) {
  aggregate(df[, -1], by = list(Sample = df$Sample), mean, na.rm = TRUE)
})

# Step 6: Write Aggregated Data to CSV Files
output_dir <- "data/aggregated"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Write aggregated non-standard samples to CSV files
for (i in seq_along(aggregate_non_standard)) {
  file_name <- paste0("non_standard_aggregated_", tools::file_path_sans_ext(basename(clean_files[i])), ".csv")
  write.csv(aggregate_non_standard[[i]], file.path(output_dir, file_name), row.names = FALSE)
}

# Write aggregated standard samples to CSV files
for (i in seq_along(aggregate_standard)) {
  file_name <- paste0("standard_aggregated_", tools::file_path_sans_ext(basename(clean_files[i])), ".csv")
  write.csv(aggregate_standard[[i]], file.path(output_dir, file_name), row.names = FALSE)
}
